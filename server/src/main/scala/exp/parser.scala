...
private val expParser: Parser[UntypedExp] =
  ...
  def operator(op: String): Parser[Location] =
    whitespace.* *> string(op).withLocation.map(_._2) <* whitespace.*
  ...
  def cond: Parser[UntypedExp] =
    (operator("if") ~ exp ~ operator("then") ~ exp ~ operator("else") ~ exp).withLocation.map {
      case ((((((ifLoc, cond), thenLoc), ifTrue), elseLoc), ifFalse), loc) =>
        Cond(cond, ifTrue, ifFalse, loc, ifLoc, thenLoc, elseLoc)
    }
  ...
  def eq: Parser[UntypedExp] =
    val compound = (add ~ operator("=") ~ eq).withLocation.map { case (((lhs, opLoc), rhs), loc) =>
      Eq(lhs, rhs, loc, opLoc)
    }
    compound | add

  // Numeric addition.
  def add: Parser[UntypedExp] =
    val compound = (paren ~ operator("+") ~ add).withLocation.map { case (((lhs, opLoc), rhs), loc) =>
      Add(lhs, rhs, loc, opLoc)
    }
